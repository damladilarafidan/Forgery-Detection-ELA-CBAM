...
from keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, Multiply, Add, Activation, Concatenate, Lambda
from keras.callbacks import EarlyStopping
from keras.utils import to_categorical
from keras.layers import Layer  # CBAM class için
...
class CBAM(Layer):
    def __init__(self, channel, ratio=8, **kwargs):
        ...
        self.shared_dense_one = Dense(max(1, self.channel // self.ratio),
                                      activation='relu', kernel_initializer='he_normal')
        self.shared_dense_two = Dense(self.channel, kernel_initializer='he_normal')

       
        self.conv_spatial = Conv2D(filters=1, kernel_size=7, strides=1,
    ...
    def call(self, input_feature):
        ...
        ch_att = Activation('sigmoid')(Add()([avg_pool, max_pool]))        
        ch_refined = Multiply()([input_feature, ch_att])                  

        # -------- Spatial Attention --------
        avg_pool = Lambda(lambda x: tf.reduce_mean(x, axis=3, keepdims=True))(ch_refined)  
        max_pool = Lambda(lambda x: tf.reduce_max(x, axis=3, keepdims=True))(ch_refined)   
        concat = Concatenate(axis=3)([avg_pool, max_pool])                                  

        sp_att = self.conv_spatial(concat)                                
        out = Multiply()([ch_refined, sp_att])                            
...
real_image_path ='CASIA_v2.0/Au/Au_ani_0001.jpg'
...
X_test, X_val, Y_test, Y_val = train_test_split(X_rest, Y_rest, test_size=0.2, random_state=42, stratify=Y_rest)
del X_rest
del Y_rest
...
model.save("myModelCBAM2.h5")


json_file = model.to_json()
with open('./mymodelCBAM.json', "w") as file:
...
dot_str = re.sub(r'(?i)\bbgcolor\s*=\s*"(?:#000000|black)"', 'bgcolor="white"', dot_str)
dot_str = re.sub(r'(?i)\bfillcolor\s*=\s*"(?:#000000|black)"', 'fillcolor="white"', dot_str)

dot_str = re.sub(r'(?i)(<FONT[^>]*\sCOLOR\s*=\s*")(?:#ffffff|white)(")', r'\1black\2', dot_str)
...
g.write_png('model_architecture_white.png')
